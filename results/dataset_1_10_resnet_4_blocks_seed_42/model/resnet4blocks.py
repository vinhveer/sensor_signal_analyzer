...
class ResNet1D(nn.Module):
    def __init__(self, in_channels: int=2, n_feature_maps: int=64, num_classes: int=4):
        ...
        self.block3 = ResidualBlock(n_feature_maps * 2, n_feature_maps * 4)
        self.block4 = ResidualBlock(n_feature_maps * 4, n_feature_maps * 8)
        final_channels = n_feature_maps * 8
        self.gap = nn.AdaptiveAvgPool1d(1)
        self.flatten = nn.Flatten()
        self.out = nn.Linear(final_channels, num_classes)
